<?php
        ...
        class URLEncoder{
            ...
            public function shortenURL(string $originURL){
                ...
                $savedID = $tableID;
                $shortURL = "";
                //This loop constructs the new shorten URL
                while ($tableID > 1){
                    $modLeft = $tableID % 36;
                    $tableID = ($tableID - $modLeft) / 36;
                    $shortURL = $shortURL. base_convert($modLeft, 10,36);
                }
            ...
            public function directURL(string $shortURL){
                ...
                $loopCount = strlen($shortURL) - 1;
                $tempID = 1;
                while($loopCount >= 0){
                    $tempURL = base_convert($shortURL[$loopCount], 36, 10);
                    $tempID = $tempID * 36 + $tempURL;
                    $tableID = $tempID;
                    $loopCount--;
                }
